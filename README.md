...
以 `piaoyizy/openwrt-x86:latest` 为基础，加入 Landscape 官方最新正式 Release 的 `redirect_pkg_handler-x86_64-static`，作为 Landscape IPv4/IPv6 流出口。保留原镜像的 OpenWrt、LuCI 和代理插件，不自行重编译固件，不内置订阅或节点。
...
局域网设备，网关保持 10.10.10.1
  ...
  -> OpenWrt firewall4 / PassWall 或 OpenClash
  -> Docker bridge -> Landscape 正常 WAN 出口
```

这里选用 **route 模式**，不是把所有流量强制送到固定 `12345` 端口。代理插件自行管理透明代理端口、策略路由和防火墙；同时启用多个透明代理插件会冲突。接应程序启动或注册成功，并不表示订阅、代理节点或 IPv6 代理已经工作。
...
确认 `8080/8443/2222` 和上述独立网段没有被占用，有冲突只调整 Compose；不要把 `10.10.10.1` 配给容器。管理端口只绑定宿主机 LAN IPv4，不映射到公网 IPv6，也不映射 53 端口。
...
5. 在 LuCI 中选择一个代理插件导入订阅并启用。ACL 必须覆盖原始客户端网段 `10.10.10.0/24` 及实际 LAN IPv6 前缀，不能只覆盖容器的 `172.30.80.0/24`。
...
- ULA 不是公网 IPv6。宿主机仍必须具有可用的公网 IPv6 默认路由，以及 Docker bridge 的 IPv6 转发/地址转换能力。宿主机 LAN 能访问 IPv6，不能证明容器也能访问 IPv6。
...
健康检查仅代表本地服务、路由、防火墙正常，**不验证** Landscape 注册或代理连通性。GitHub CI 另行验证模拟注册 socket、双栈 LuCI、带外网段源地址的 NAT44/NAT66 和配置卷重建。CI 不是你的真实路由系统，无法替代 LAN → Landscape 标记流 → 代理节点的端到端验收。
